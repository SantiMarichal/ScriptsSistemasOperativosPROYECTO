	...
	while [ "$opc1" -ne 0 ]; do
		...
		1)
			...
			while [ "$opc2" -ne 0 ]; do
				...
				1)
					echo "Ha ingresado la opcion 1: "
					read -p "¿Quiere bloquear un rango de puertos o solo un puerto? (1- Rango / 2- Solo un puerto / 0- Volver): " opc3
					while [[ "$opc3" -ne 0 ]]; do
						case $opc3 in
						1)
				...
				2)
					...
					read -p "¿Quiere bloquear un rango de puertos o solo un puerto? (1- Rango / 2- Solo un puerto / 0- Volver): " opc4
					while [[ "$opc4" -ne 0 ]]; do
						case $opc4 in
						1)
							read -p "Ingrese el puerto inicial: " puerto_inicial
		...
		2)
			...
			while [ "$opc5" -ne 0 ]; do
				...
				1)
					echo "Ha ingresado la opcion 1: "
					read -p "¿Quiere desbloquear un rango de puertos o solo un puerto? (1- Rango / 2- Solo un puerto / 0- Volver): " opc6
					while [[ "$opc6" -ne 0 ]]; do
						case $opc6 in
						1)
							read -p "Ingrese el puerto inicial: " puerto_inicial
				...
				2)
					echo "Ha ingresado la opcion 2: "
					read -p "¿Quiere desbloquear un rango de puertos o solo un puerto? (1- Rango / 2- Solo un puerto / 0- Volver): " opc7
					while [[ "$opc7" -ne 0 ]]; do
						case $opc7 in
						1)
							read -p "Ingrese el puerto inicial: " puerto_inicial
...
bloquearIPyPuerto() {
	...
	read -p "¿Quiere que sea permanente o temporal? 1- Permanente / 2- Temporal / 0- Volver: " opc11
	while [[ "$opc11" -ne 0 ]]; do
		case $opc11 in
		1)
			echo "Ha ingresado la opcion 1: "
			read -p "Ingrese la ip a bloquear: " ip2
			read -p "Ingrese el puerto a bloquear: " puerto2
			read -p "Ingrese el protocolo (tcp/udp): " protocolo2
			if sudo firewall-cmd --permanent --add-rich-rule="rule family='ipv4' source address='$ip2' port protocol='$protocolo2' port='$puerto2' reject"; then
				sudo firewall-cmd --reload
				echo "IP y puerto bloqueadas correctamente."
			else
				echo "Error al bloquear la IP."
			fi
			;;
		2)
			echo "Ha ingresado la opcion 2: "
			read -p "Ingrese la ip a bloquear: " ip3
			read -p "Ingrese el puerto a bloquear: " puerto3
			read -p "Ingrese el protocolo (tcp/udp): " protocolo3
			if sudo firewall-cmd --add-rich-rule="rule family='ipv4' source address='$ip3' port protocol='$protocolo3' port='$puerto3' reject"; then
				echo "IP y puerto bloqueadas correctamente."
			else
				echo "Error al bloquear la IP."
			fi
